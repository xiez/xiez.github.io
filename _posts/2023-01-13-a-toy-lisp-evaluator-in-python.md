...
首先，解析器作为解释器的「前端」，主要负责把用户输入的代码转换成内部的数据结构，也就是所谓的抽象语法树（AST）。我们沿用[上篇文章](https://blog.justinzx.com/lisp-expression-parsing/)里的 parser，最后出来的 AST 如下：
...
解释-应用循环是解释器的核心，由两个函数 `eval` 和 `apply`组成。
...
最后，完整的代码请参考 [lisp_evaluator.py](https://github.com/xiez/SICP-exercises/blob/master/python/lisp_evaluator.py)。

“八皇后”代码请参考测试用例 [test-cases/queens.ss](https://github.com/xiez/SICP-exercises/blob/master/python/test-cases/queens.ss)，除了增加一些辅助函数（append, length, map, filter 等），其他基本从[练习2.42](https://github.com/xiez/SICP-exercises/blob/master/2.42/exer.ss)拷贝过来。
