...
分布式系统的「共识问题」（consensus）是指多个节点共享数据，并通过共识算法来确数据的一致性。
...
![state-machine](../../assets/images/2024/04/state-machine.jpg "state-machine")
...
节点初始化完成后，设置一个计时器，过期时间（`election_timeout`）为 150ms-300ms 之间的随机值。

1. 在计时器过期时间内，如果当前状态或者任期发生变化，则停止计时器。
...
触发选举后，节点状态置为“candidate”后，先投自己一票，再向所有节点发送投票请求，最后再设置一个计时器，过期时间可以跟 `electionTimeout` 相同。

1. 在计时器过期时间内，收到超过半数的投票，则成为领导者。

2. 如果计时器过期了，则重置计时器，开始下一轮投票。
...
节点在收到领导者的心跳消息后，先检查自己当前任期是否过期，如果过期了，降级成跟随者；如果已经是追随者了，就重置计时器；其他状态节点可以忽略该消息。
...
![new leader](../../assets/images/2024/04/raft-new-leader.jpg "new leader")
...
这种多余的选举目前无法避免，但在后续引入了「日志复制」（log replication）后，可以有效解决。
